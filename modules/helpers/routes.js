...
exports.makeRoutes = (baseRoutes, {
  ...
  const buildLocalizedRoutes = (route, routeOptions = {}, isChild = false, isExtraRouteTree = false) => {
    ...
    const generateLocales = [...componentOptions.locales];
    if (strategy === STRATEGIES.PREFIX_ALSO_DEFAULT) {
      generateLocales.push('default');
    }
    // Generate routes for component's supported locales
    for (let i = 0, length1 = generateLocales.length; i < length1; i++) {
      const locale = generateLocales[i]
      const { name } = route
      let { path } = route
