 ...
 */
export const createLocaleFromRouteGetter = (localeCodes, { routesNameSeparator, defaultLocaleRouteNameSuffix, checkLocaleDefault }) => {
  const localesPattern = `(${localeCodes.join('|')})` // (zh-cn|en-us)
  const defaultSuffixPattern = `(?:${routesNameSeparator}${defaultLocaleRouteNameSuffix})?` // (?:__default)
  const regexpName = new RegExp(`${routesNameSeparator}${localesPattern}${defaultSuffixPattern}$`, 'i') // __(zh-cn|en-us)(?:__default)$
  const regexpDefaultName = new RegExp(`${routesNameSeparator}${defaultLocaleRouteNameSuffix}$`, 'i') // __default$
  const regexpPath = new RegExp(`^/${localesPattern}/`, 'i'); // ^/(zh-cn|en-us)/
  ...
  const getLocaleFromRoute = (route) => {
    ...
    if (route.name) {
      ...
      if (checkLocaleDefault) {
        const matches = route.name.match(regexpDefaultName);
        if (matches) {
          return 'default'
        }
      }
 ...
 */
export const setLocaleCookie = (locale, res, { useCookie, cookieDomain, cookieKey, sameSite }) => {
  if (!useCookie) {
    return
  }
  const date = new Date()
