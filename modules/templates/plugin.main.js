...
import {
  onDefaultDetectLanguage,
  onDefaultSetLanguage
} from './custom';

Vue.use(VueI18n)

const checkLocaleDefault = strategy === STRATEGIES.PREFIX_ALSO_DEFAULT;
const getLocaleFromRoute = createLocaleFromRouteGetter(localeCodes, { routesNameSeparator, defaultLocaleRouteNameSuffix, checkLocaleDefault })
...
export default async (context) => {
  ...
  // newLocale can be placeholder of 'default'
  const loadAndSetLocale = async (newLocaleOrDefault, { initialSetup = false } = {}) => {
    // Abort if different domains option enabled
    if (!initialSetup && app.i18n.differentDomains) {
      return
    }

    if (newLocaleOrDefault === app.i18n.localeOrDefault) {
      return;
    }

    console.log(`[ChangeLocaleOrDefault] ${app.i18n.localeOrDefault} => ${newLocaleOrDefault}`);

    const newLocale = newLocaleOrDefault === 'default' ? onDefaultDetectLanguage(context) || app.i18n.defaultLocale: newLocaleOrDefault;
    ...
    if (newLocale !== oldLocale) {
      ...
      if (strategy === STRATEGIES.PREFIX_ALSO_DEFAULT) {
        onDefaultSetLanguage(context, newLocale);
      }
    ...
    app.i18n.localeOrDefault = newLocaleOrDefault;

    await syncVuex(store, newLocale, app.i18n.getLocaleMessage(newLocale), { vuex })

    // Must retrieve from context as it might have changed since plugin initialization.
    const { route } = context
    const redirectPath = getRedirectPathForLocale(route, newLocaleOrDefault)
    ...
    if (initialSetup) {
      ...
      if (redirectPath) {
        console.log(`[RedirectPathInSetLocale] ${route.fullPath} => ${redirectPath}`)
        redirect(redirectPath)
      }
    }
  }

  const getRedirectPathForLocale = (route, localeOrDefault) => {
    if (!localeOrDefault || app.i18n.differentDomains || strategy === STRATEGIES.NO_PREFIX) {
      return ''
    }

    if (getLocaleFromRoute(route) === localeOrDefault) {
      return ''
    }

    // At this point we are left with route that either has no or different locale.
    let redirectPath = app.switchLocalePath(localeOrDefault)

    if (!redirectPath) {
      // Current route could be 404 in which case attempt to find matching route for given locale.
      redirectPath = app.localePath(route.fullPath, localeOrDefault)
  ...
  app.i18n.locale = ''; // real locale
  app.i18n.localeOrDefault = '';
